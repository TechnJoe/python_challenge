...
with open (csvpath) as csvfile:
    ...
    for row in csvreader:
        ...
        #candidates  listed are Khan, Correy, Li, and O'Tooley
        candidate =""
        candidate_list = []
        Total_votes_cast = 0
        candidate_votes = 0
    ...
    output +=("Total votes cast: {Total_votes_cast}\n")
    output +=(f"Correy: {Correy_percent}% ({Correy})\n")
    output +=(f"Li: {Li_percent}% ({Li})\n")
    output +=(f"O'Tooley: {Otooley_percent}% ({Otooley})\n")
    output +=(f"Khan: {Khan_percent}% ({Khan})\n")
